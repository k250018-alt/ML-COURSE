...
class KNearestNeighbour(Alogrothm):
    def __init__(self):
    ...
    def voting(self, nearest_neighbours, Ztrain):
        #what majorty of k nearest neibours
        counter = Counter(Ztrain[nearest_neighbours])
        y_pred  = counter.most_common()[0][0]
        return y_pred
    ...
    def test(self, Xtest):
        ...
        if self.__model == 1:
            ...
            return self.voting(nearest_neighbours, self.__Ztrain)
        if self.__model == 2:
    ...
    def check_accuracy(self,X,z):
        pred = self.predict(X)
        return np.mean(np.equal(pred, z))